...
public interface ExportadorListaProduto {
    ...
    static ExportadorListaProduto newInstance(String extensaoArqExportacao){
        var classSuffix = extensaoArqExportacao.toLowerCase();
        classSuffix = Character.toUpperCase(classSuffix.charAt(0)) + classSuffix.substring(1);
        var fullClassName = ExportadorListaProduto.class.getName() + classSuffix;

        try {
            Class aClass = Class.forName(fullClassName);
            Constructor<?> constructor = aClass.getDeclaredConstructor();
            return (ExportadorListaProduto) constructor.newInstance();
        } catch (ClassNotFoundException e) {
            throw new RuntimeException("Formato não suportado.", e);
        } catch (NoSuchMethodException | InvocationTargetException | InstantiationException | IllegalAccessException e) {
            throw new RuntimeException("Nao há um construtor padrao", e);
        }
    }
}
